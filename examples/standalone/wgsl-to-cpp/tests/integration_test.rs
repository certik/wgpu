...
#[test]
fn test_bezier_shader_translation() {
    let wgsl_source = r#"
struct Uniforms {
    resolution: vec2<f32>,
    p0: vec2<f32>,
...
"#;
    ...
    let cpp_code =
        translate_wgsl_to_cpp(wgsl_source).expect("Failed to translate Bezier shader WGSL");

    assert!(
        !cpp_code.contains("/* expr */") && !cpp_code.contains("// Unsupported statement"),
        "Generated C++ contains placeholders for unsupported features:\n{}",
        cpp_code
    );
}
